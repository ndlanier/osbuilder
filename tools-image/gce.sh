...
mv $RAW /artifacts/disk.raw

GB=$((1024*1024*1024))
size=$(qemu-img info -f raw --output json /artifacts/disk.raw | gawk 'match($0, /"virtual-size": ([0-9]+),/, val) {print val[1]}')
# shellcheck disable=SC2004
ROUNDED_SIZE=$(echo "$size/$GB+1"|bc)
echo "Resizing raw image from \"$size\"MB to \"$ROUNDED_SIZE\"GB"
qemu-img resize -f raw /artifacts/disk.raw "$ROUNDED_SIZE"G
echo "Compressing raw image disk.raw to $OUT.tar.gz"
tar -c -z --format=oldgnu -f "$OUT".tar.gz /artifacts/disk.raw